  ...
  Stream<List<MyOrderEntity>> getAll() async*{
    ...
    final orders = dd;
     yield[
      ...
      MyOrderEntity(id: data.id, uid: data.uid, location: data.location, time: data.time, 
      items: [
        for(final type in data.items)
        OrderItemEntity(productId: type.productId, type: type.type, quantity: type.quantity)
      ])
    ];
    }
